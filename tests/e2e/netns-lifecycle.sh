...
cleanup() {
  ...
  test -z "${pid_duplicate}" || kill "${pid_duplicate}" 2>/dev/null || true
...
wait_route() {
  ...
  while :; do
    # babeld deliberately installs a temporary unreachable route after a
    # retraction (RFC 8966 overlap protection); that is not an active path.
    route=$(ip -n "${ns}" -6 route show table "${table}" exact "${prefix}" 2>/dev/null | grep -v '^unreachable ' || true)
...
prefix_shutdown=2001:db8:740::/64
...
# Route protocol is an exclusive ownership token within one network namespace.
ip netns exec "${ns_rs}" "${daemon}" run --config "${runtime}/rs.toml" \
  --control-socket "${runtime}/duplicate.ctl" >"${runtime}/duplicate.log" 2>&1 &
pid_duplicate=$!
sleep 1
if kill -0 "${pid_duplicate}" 2>/dev/null; then
  echo "second daemon unexpectedly acquired the same route protocol" >&2
  exit 1
fi
if wait "${pid_duplicate}"; then
  echo "second daemon unexpectedly exited successfully" >&2
  exit 1
fi
pid_duplicate=
grep -q 'already owned' "${runtime}/duplicate.log"
...
sleep 2
kill -0 "${pid_rs}"
wait_route "${ns_peer}" 201 "${prefix_shutdown}" yes 5
...
wait "${pid_rs}"
pid_rs=
wait_route "${ns_peer}" 201 "${prefix_shutdown}" no 8
