...
and are not claimed. The deployment described here runs Babel on authenticated
...
| BABEL-OUTPUT-01 | 8966 §3.1 | Periodic, triggered, urgent, and explicit Ack deadlines are retained as typed engine output; jitter never exceeds its deadline | `rfc8966_3_1_output_actions_preserve_their_deadlines`, output scheduler tests |
| BABEL-OUTPUT-02 | 8966 §3.1 | Compatible TLVs aggregate during bounded jitter; datagrams are paced unless their deadline requires an earlier send | output scheduler aggregation, deterministic jitter, packetisation, and pacing tests |
...
| MANUAL-LINK-01 | RFC 8966 §3.7.4 permits split horizon only on symmetric, transitive links; configuration cannot prove those physical properties. | Confirm that `wired` and `tunnel` interfaces are symmetric and transitive, or explicitly disable split horizon. The `wireless` preset disables it. |
...
- The Linux exporter rejects overlapping nonzero source prefixes. This is a
  deliberate deployment restriction until a general destination-first
